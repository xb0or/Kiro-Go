...
func (h *Handler) disableAccountOverage(account *config.Account) {
	...
	// Fork 模式下不再保留本地 AllowOverage；遇到 402 OVERAGE 时刷新上游
	// OverageStatus 缓存，由调度逻辑直接根据 OverageStatus 判定是否跳过。
	go func(acc config.Account) {
		snap, fetchErr := FetchOverageStatus(&acc)
		if fetchErr != nil {
			logger.Warnf("[AccountFailover] refresh overage status failed for %s: %v", acc.Email, fetchErr)
			return
		}
		if persistErr := PersistOverageSnapshot(acc.ID, snap); persistErr != nil {
			logger.Warnf("[AccountFailover] persist overage snapshot failed for %s: %v", acc.Email, persistErr)
			return
		}
		logger.Warnf("[AccountFailover] Refreshed upstream overage status for %s after 402 OVERAGE", acc.Email)
		h.pool.Reload()
	}(*account)
}
